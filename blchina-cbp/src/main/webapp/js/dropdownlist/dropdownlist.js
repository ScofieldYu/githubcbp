...
/*$.dropdownlist({
    ...
    defTit: undefined,   //默认显示值
    defValue: undefined,   //要显示的字段
    ...
    $.extend({
        dropdownlist: function (config) {
            ...
            var dropObj = {
                ...
                , bindData: function () {
                    var thiz = this;
                    // 如果传人默认字段,则text按照默认字段渲染
                    var text = 'text';
                    if(thiz.config.defValue){
                        text = thiz.config.defValue
                    }
                    ...
                    ul.html(lis);
                    //默认显示
                    thiz.config.container.find("label").text(thiz.config.defTit || '请选择').attr("val", "");
                    //显示当前值
